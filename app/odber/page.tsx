export const metadata = {
  title: "Odoberať newsletter | ZAJO Reality",
  description: "Dostávajte najlepšie ponuky nehnuteľností v Trenčianskom kraji skôr ako sa objavia inde. Zadarmo, odber kedykoľvek zrušíte.",
};
...
export default function OdberPage() {
  return (
    <>
      <style>{`
:root{--navy:#0F172A;--gold:#F59E0B;--gold-dark:#D97706;--gold-light:#FEF3C7;--white:#FFFFFF;--gray-50:#F8FAFC;--gray-400:#94A3B8;--gray-600:#475569;--gray-700:#334155;--green:#059669;--ease-out:cubic-bezier(0.23,1,0.32,1)}
*,*::before,*::after{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}
body{font-family:'Inter',sans-serif;background:var(--white);color:var(--navy);-webkit-font-smoothing:antialiased}
nav{display:none}
.hero-logo{margin-bottom:14px}.hero-logo svg{height:62px;width:auto;display:block;margin:0 auto}
.hero{background:linear-gradient(160deg,#060E1A 0%,#0F172A 55%,#1a2744 100%);min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:28px 24px 48px;text-align:center;position:relative;overflow:hidden}
...
.deco-cards{position:absolute;inset:0;pointer-events:none;overflow:hidden}.deco-card{position:absolute;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:12px}
.deco-card:nth-child(1){width:180px;height:110px;top:12%;left:5%;transform:rotate(-8deg)}
.deco-card:nth-child(2){width:140px;height:90px;top:22%;right:7%;transform:rotate(6deg)}
.deco-card:nth-child(3){width:160px;height:100px;bottom:20%;left:8%;transform:rotate(5deg)}
.deco-card:nth-child(4){width:120px;height:80px;bottom:28%;right:5%;transform:rotate(-4deg)}
.hero-badge{display:inline-flex;align-items:center;gap:7px;background:rgba(245,158,11,0.14);border:1px solid rgba(245,158,11,0.3);color:var(--gold);font-size:.74rem;font-weight:700;padding:5px 14px;border-radius:99px;text-transform:uppercase;letter-spacing:.06em;margin-bottom:12px;animation:fadeUp .5s var(--ease-out) both;position:relative;z-index:1}
.hero h1{font-family:'Playfair Display',serif;font-size:clamp(1.6rem,4.5vw,2.8rem);font-weight:700;color:var(--white);line-height:1.1;letter-spacing:-.025em;max-width:700px;margin:0 auto 12px;animation:fadeUp .5s 60ms var(--ease-out) both;position:relative;z-index:1}
.hero h1 em{font-style:normal;color:var(--gold)}
.hero-sub{font-size:.92rem;color:rgba(255,255,255,.62);max-width:480px;margin:0 auto 8px;line-height:1.55;animation:fadeUp .5s 100ms var(--ease-out) both;position:relative;z-index:1}
.hero-fomo{display:inline-flex;align-items:center;gap:6px;font-size:.78rem;font-weight:700;color:var(--gold);margin-bottom:18px;animation:fadeUp .5s 140ms var(--ease-out) both;position:relative;z-index:1}
.hero-fomo svg{animation:pulse 2s ease-in-out infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}}
.signup-card{background:var(--white);border-radius:20px;padding:16px 32px 24px;box-shadow:0 24px 80px rgba(0,0,0,.45);max-width:480px;width:100%;position:relative;z-index:2;animation:fadeUp .5s 200ms var(--ease-out) both}
@media(max-width:480px){.signup-card{padding:18px 18px 22px;border-radius:16px}}
.card-icon{width:40px;height:40px;background:var(--gold-light);border-radius:11px;display:flex;align-items:center;justify-content:center;color:var(--gold-dark);margin-bottom:10px}
.signup-card h2{font-family:'Playfair Display',serif;font-size:1.35rem;font-weight:700;color:var(--navy);margin-bottom:4px}
.signup-card>p{font-size:.9rem;color:var(--gray-600);margin-bottom:14px;line-height:1.5}
.perks{display:flex;flex-direction:column;gap:6px;margin-bottom:14px}
.perk{display:flex;align-items:center;gap:10px;font-size:.84rem;color:var(--gray-700)}
.perk-icon{width:28px;height:28px;flex-shrink:0;background:var(--gold-light);border-radius:8px;display:flex;align-items:center;justify-content:center;color:var(--gold-dark)}
...
.field-error{font-size:.72rem;color:#EF4444;display:none}.field-error.show{display:block}
.submit-btn{width:100%;background:var(--gold);color:var(--navy);font-family:'Inter',sans-serif;font-size:1rem;font-weight:700;padding:14px 24px;border:none;border-radius:12px;cursor:pointer;margin-top:4px;display:flex;align-items:center;justify-content:center;gap:8px;transition:background 180ms var(--ease-out),transform 160ms var(--ease-out),box-shadow 200ms var(--ease-out)}
@media(hover:hover){.submit-btn:hover{background:var(--gold-dark);box-shadow:0 8px 24px rgba(245,158,11,.35)}}
.submit-btn:active{transform:scale(.97)}.submit-btn:disabled{opacity:.65;cursor:not-allowed;transform:none}
...
.submit-btn.loading .btn-text{display:none}.submit-btn.loading .spinner{display:block}
...
.form-note{text-align:center;font-size:.7rem;color:var(--gray-400);margin-top:10px;display:flex;align-items:center;justify-content:center;gap:5px}
.success-view{display:none;text-align:center;padding:12px 0}.success-view.show{display:block}
.form-fields.hide{display:none}
.success-icon{width:64px;height:64px;background:#D1FAE5;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 18px;animation:popIn .4s var(--ease-out) both}
.success-icon svg{color:var(--green)}
.success-view h3{font-family:'Playfair Display',serif;font-size:1.4rem;color:var(--navy);margin-bottom:8px}
.success-view p{font-size:.875rem;color:var(--gray-600);line-height:1.6}
.why{background:#060E1A;padding:64px 24px}.why-inner{max-width:820px;margin:0 auto;text-align:center}
...
.why-item{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:14px;padding:20px;text-align:left;display:flex;align-items:flex-start;gap:16px;transition:background 200ms var(--ease-out),border-color 200ms var(--ease-out)}
@media(min-width:600px){.why-item{flex-direction:column;gap:0}}
@media(hover:hover){.why-item:hover{background:rgba(245,158,11,.07);border-color:rgba(245,158,11,.2)}}
.why-item-icon{color:var(--gold);margin-bottom:0;flex-shrink:0}
@media(min-width:600px){.why-item-icon{margin-bottom:12px}}
.why-item h3{font-size:.95rem;font-weight:600;color:var(--white);margin-bottom:4px}
.why-item p{font-size:.82rem;color:rgba(255,255,255,.55);line-height:1.6}
footer{background:var(--navy);color:rgba(255,255,255,.4);text-align:center;padding:26px 24px;font-size:.78rem}
...
@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms!important;transition-duration:.01ms!important}}
      ...
      <section className="hero">
        <div className="deco-cards">
          <div className="deco-card" /><div className="deco-card" /><div className="deco-card" /><div className="deco-card" />
        </div>
        ...
        <div className="signup-card">
          ...
          <div className="perks">
            {[
              ...
              "Max. 1× mesačne, odber kedykoľvek zrušíte",
            ].map((text) => (
          ...
          <div className="form-fields" id="formFields">
            <div className="form-group">
              ...
              <input type="text" id="meno" name="meno" placeholder="Ján" autoComplete="given-name" />
            </div>
            <div className="form-group">
              <label htmlFor="email">E-mailová adresa *</label>
              <input type="email" id="email" name="email" placeholder="jan@email.sk" autoComplete="email" />
              <span className="field-error" id="err-email">Zadajte platnú e-mailovú adresu</span>
            </div>
            ...
            <button type="button" className="submit-btn" id="submitBtn">
        ...
        <div className="why-inner">
          ...
          <div className="why-grid">
            <div className="why-item">
              ...
              <div><h3>Skôr ako ostatní</h3><p>Niektoré ponuky posielame odberateľom ešte pred zverejnením na Nehnuteľnosti.sk a Reality.sk.</p></div>
            </div>
            <div className="why-item">
              ...
              <div><h3>Trendy trhu</h3><p>Prehľad vývoja cien v lokalitách Trenčianskeho kraja — viete kedy je správny čas kupovať.</p></div>
            </div>
            <div className="why-item">
              ...
              <div><h3>Bez spamu</h3><p>Maximálne 1× mesačne. Len to najlepšie, žiadne zbytočné e-maily. Odber zrušíte jedným klikom.</p></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer>
        <p>© 2025 Zajo Reality · <a href="mailto:info@zajoreality.sk">info@zajoreality.sk</a> · <a href="tel:+421907980436">+421 907 980 436</a></p>
      </footer>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        (function() {
          ...
          btn.addEventListener('click', async function() {
            ...
            var menoEl  = document.getElementById('meno');
            var errEl   = document.getElementById('err-email');
            var gdprEl  = document.getElementById('gdprSuhlas');
            ...
            if (gdprEl && !gdprEl.checked) {
              var eg = document.getElementById('err-gdpr');
              if (!eg) { eg = document.createElement('span'); eg.id = 'err-gdpr'; eg.style.cssText = 'font-size:.65rem;color:#ef4444;margin-top:2px;display:block;'; gdprEl.closest('label').appendChild(eg); }
              eg.textContent = 'Toto pole je povinné'; return;
            }
            if (gdprEl) { var eg2 = document.getElementById('err-gdpr'); if (eg2) eg2.style.display = 'none'; }
            ...
            try {
              var res = await fetch('/api/subscribe', {
                ...
                body: JSON.stringify({ name: menoEl.value.trim() || 'Odberateľ', email: emailEl.value.trim(), source: 'odber_landing' })
              });
              if (!res.ok && res.status !== 409) throw new Error();
            } catch(e) {
              btn.classList.remove('loading'); btn.disabled = false;
              alert('Nastala chyba. Skúste prosím znova.'); return;
            }
