...
export async function GET() {
  ...
  let query = supabaseAdmin.from('leads_predaj').select('*').order('created_at', { ascending: false })
  // A maklér only sees their own leads + the shared (unassigned) pool.
  ...
  if (data.email) {
    ...
    if (newsletter_opt) {
      try {
        ...
        await logConsent({ email: data.email!, name: data.name, action: 'opt_in', source: 'predaj_form', actor: 'self' })
